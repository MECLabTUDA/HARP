...
def diagonal_corner(img_shape):
    random_corner = ["top_left", "bottom_left", "top_right", "bottom_right"]
    random_corner = random.choice(random_corner)
    h, w = img_shape[:2]
    mask = np.ones((h,w))
    ...
    for y in range(h):
        for x in range(w):
            if random_corner=="top_left":
                if x + y < w:    
                    mask[x,y]=1
            elif random_corner=="bottom_left":
                if x > y:    
                    mask[x,y]=1
            elif random_corner=="top_right":
                if x < y:    
                    mask[x,y]=1
            elif random_corner=="bottom_right":   
                if x + y > w:    
                    mask[x,y]=1
    mask = mask[:, :, None].astype('uint8')
    return mask

def box_corner(img_shape):
    h, w = img_shape[:2]
    random_corner = [(0,0,h//2,w//2), (h//2,0,h,w//2), (0,w//2,h//2,w), (h//2,w//2,h,w)]
    random_corner = random.choice(random_corner)
    mask = bbox2mask(img_shape, random_corner)
    mask = np.array(mask).astype('uint8')
    return mask

def empty_mask(img_shape):
    h, w = img_shape[:2]
    mask = np.ones((h,w))
    mask = mask[:, :, None].astype('uint8')
    return mask
